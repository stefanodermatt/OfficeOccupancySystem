(function () {
    ...
    angular
        .module('app.office1')
        .controller('Office1Controller', Office1Controller);

    Office1Controller.$inject = ['DataService'];

    function Office1Controller(DataService) {
        var vm = this;
        ...
        function fetchAndDisplayData() {
            ...
            if (peopleInTheOffice <= 0)
                vm.currentOccupancy = 'Currently, nobody is in the Bern office.';
            else if (peopleInTheOffice === 1)
                vm.currentOccupancy = 'Currently, 1 person is in the Bern office.';
            else
                vm.currentOccupancy = 'Currently, ' + peopleInTheOffice + ' people are in the Bern office.';
        ...
        function drawLineChart( elementId, data ) {
            ...
            svg.append( 'g' )
                .attr( 'class', 'lineChart1--xAxisTicks' )
                .attr( 'transform', 'translate(' + detailWidth / 2 + ',' + height + ')' )
                .call( xAxisTicks );

            svg.append( 'g' )
                .attr( 'class', 'lineChart1--xAxis' )
                .attr( 'transform', 'translate(' + detailWidth / 2 + ',' + ( height + 7 ) + ')' )
                .call( xAxis );

            svg.append( 'g' )
                .attr( 'class', 'lineChart1--yAxisTicks' )
                .call( yAxisTicks );

            // Add the line path.
            svg.append( 'path' )
                .datum( startData )
                .attr( 'class', 'lineChart1--areaLine' )
                .attr( 'd', line )
                .transition()
                .duration( DURATION )
                .delay( DURATION / 2 )
                ...
                circleContainer.datum( datum )
                    ...
                    .on( 'mouseenter', function( d ) {
                        d3.select( this )
                            .attr(
                            'class',
                            'lineChart1--circle lineChart1--circle__highlighted'
                        )
                            .attr( 'r', 7 );
            ...
            function hideCircleDetails() {
                circleContainer.selectAll( '.lineChart1--bubble' )
                    .remove();
            }

            function showCircleDetail( data ) {
                var details = circleContainer.append( 'g' )
                    .attr( 'class', 'lineChart1--bubble' )
                    .attr(
                    'transform',
                    function() {
                ...
                var text = details.append( 'text' )
                    .attr( 'class', 'lineChart1--bubble--text' );

                text.append( 'tspan' )
                    .attr( 'class', 'lineChart1--bubble--label' )
                ...
                text.append( 'tspan' )
                    .attr( 'class', 'lineChart1--bubble--value' )
                    .attr( 'x', detailWidth / 2 )
